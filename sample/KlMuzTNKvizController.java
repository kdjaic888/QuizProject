...
public class KlMuzTNKvizController {
    ...
    static ArrayList<KlMuzTNKviz> questions;
    static int percent;
    static int pauseValue = 1500;

    @FXML
    void initialize() {
        questions = KlMuzTNKviz.loadQuestions("./Klasicnamuzikatn.txt");

        KlMuzTNKviz.setButtons(button1, button2);

        questions.get(KlMuzTNKviz.getQuestionIndex()).displayQuestion(questionLabel, questionNum);
    ...
    private void handleButtonAction(ActionEvent event) {
        ...
        questions.get(KlMuzTNKviz.getQuestionIndex()).checkCorrect((Button) event.getTarget(), questions, scoreLabel);
    ...
    public static void finished(int score, int questionsCorrect) {
        ...
        finish.getDialogPane().getStylesheets().add(KlMuzTNKviz.class.getResource("lightTheme.css").toExternalForm());
        finish.showAndWait();
