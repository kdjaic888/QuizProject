...
public class PromTNKvizController {
    ...
    static ArrayList<PromTNKviz> questions;
    static int percent;
    static int pauseValue = 1500;

    @FXML
    void initialize() {
        questions = PromTNKviz.loadQuestions("./Prometniznakovitn.txt");

        PromTNKviz.setButtons(button1, button2);

        questions.get(PromTNKviz.getQuestionIndex()).displayQuestion(questionLabel, questionNum);
    ...
    private void handleButtonAction(ActionEvent event) {
        ...
        questions.get(PromTNKviz.getQuestionIndex()).checkCorrect((Button) event.getTarget(), questions, scoreLabel);
    ...
    public static void finished(int score, int questionsCorrect) {
        ...
        finish.getDialogPane().getStylesheets().add(PromTNKviz.class.getResource("lightTheme.css").toExternalForm());
        finish.showAndWait();
