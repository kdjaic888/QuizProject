...
public class PromIzKvizController {
    ...
    static ArrayList<PromIzKviz> questions;
    static int percent;
    static int pauseValue = 1500;

    @FXML
    void initialize() {
        questions = PromIzKviz.loadQuestions("./Prometniznakoviiz.txt");

        PromIzKviz.setButtons(button1, button2, button3, button4);

        questions.get(PromIzKviz.getQuestionIndex()).displayQuestion(questionLabel, questionNum);
    ...
    private void handleButtonAction(ActionEvent event) {
        ...
        questions.get(PromIzKviz.getQuestionIndex()).checkCorrect((Button) event.getTarget(), questions, scoreLabel);
    ...
    public static void finished(int score, int questionsCorrect) {
        ...
        finish.getDialogPane().getStylesheets().add(PromIzKviz.class.getResource("lightTheme.css").toExternalForm());
        finish.showAndWait();
