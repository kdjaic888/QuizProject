...
public class PovijestTNKvizController {
    ...
    static ArrayList<PovijestTNKviz> questions;
    static int percent;
    static int pauseValue = 1500;

    @FXML
    void initialize() {
        questions = PovijestTNKviz.loadQuestions("./Povijesttn.txt");

        PovijestTNKviz.setButtons(button1, button2);

        questions.get(PovijestTNKviz.getQuestionIndex()).displayQuestion(questionLabel, questionNum);
    ...
    private void handleButtonAction(ActionEvent event) {
        ...
        questions.get(PovijestTNKviz.getQuestionIndex()).checkCorrect((Button) event.getTarget(), questions, scoreLabel);
    ...
    public static void finished(int score, int questionsCorrect) {
        ...
        finish.getDialogPane().getStylesheets().add(PovijestTNKviz.class.getResource("lightTheme.css").toExternalForm());
        finish.showAndWait();
