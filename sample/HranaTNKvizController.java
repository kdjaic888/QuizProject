...
public class HranaTNKvizController {
    ...
    static ArrayList<HranaTNKviz> questions;
    static int percent;
    static int pauseValue = 1500;

    @FXML
    void initialize() {
        questions = HranaTNKviz.loadQuestions("./Hranatn.txt");

        HranaTNKviz.setButtons(button1, button2);

        questions.get(HranaTNKviz.getQuestionIndex()).displayQuestion(questionLabel, questionNum);
    ...
    private void handleButtonAction(ActionEvent event) {
        ...
        questions.get(HranaTNKviz.getQuestionIndex()).checkCorrect((Button) event.getTarget(), questions, scoreLabel);
    ...
    public static void finished(int score, int questionsCorrect) {
        ...
        finish.getDialogPane().getStylesheets().add(HranaTNKviz.class.getResource("lightTheme.css").toExternalForm());
        finish.showAndWait();
